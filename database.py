...
# ──────────────────────────────────────────────
# SYNC POOL — для Flask (Mini App / Render)
# ──────────────────────────────────────────────
...
def get_db():
    """
    Синхронный контекстный менеджер для Flask.
    Возвращает connection с RealDictCursor (rows как dict).
    """
...
def init_db_sync():
    """Создаёт таблицы при первом запуске (вызывается при импорте webapp.py)."""
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            for c in ["Москва", "Санкт-Петербург", "Дубай", "Алматы", "Астана", "Екатеринбург"]:
                cur.execute(
                    "INSERT INTO cities (name, manager_id) VALUES (%s, 0) ON CONFLICT DO NOTHING",
                    (c,)
                )


# ──────────────────────────────────────────────
# ASYNC POOL — для Aiogram Bot (Sakura)
# ──────────────────────────────────────────────
_async_pool = None

async def get_async_pool():
    """Ленивая инициализация asyncpg пула."""
    global _async_pool
